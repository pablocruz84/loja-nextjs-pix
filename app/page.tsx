...
const enviarPedidoPorEmail = async () => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-center shrink-0">
            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center mb-1 transition-all ${
              etapa >= 1 ? 'bg-orange-500' : 'bg-gray-200'
            }`}>
              <svg className={`w-4 h-4 md:w-5 md:h-5 ${etapa >= 1 ? 'text-white' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 24 24">
                <path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.08-.14.12-.31.12-.48 0-.55-.45-1-1-1H5.21l-.94-2H1zm16 16c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2z"/>
              </svg>
            </div>
            <span className={`text-xs font-semibold ${etapa >= 1 ? 'text-gray-900' : 'text-gray-400'}`}>
              Produtos
            </span>
          </div>
          ...
          <div className="flex flex-col items-center shrink-0">
            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center mb-1 transition-all ${
              ...
              <svg className={`w-4 h-4 md:w-5 md:h-5 ${etapa >= 3 ? 'text-white' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>
              </svg>
            </div>
            <span className={`text-xs font-semibold ${etapa >= 3 ? 'text-gray-900' : 'text-gray-400'}`}>
              Dados
            </span>
          </div>
          ...
          <div className="flex flex-col items-center shrink-0">
            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center mb-1 transition-all ${
              ...
              <svg className={`w-4 h-4 md:w-5 md:h-5 ${etapa >= 4 ? 'text-white' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 24 24">
                <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/>
              </svg>
            </div>
            <span className={`text-xs font-semibold ${etapa >= 4 ? 'text-gray-900' : 'text-gray-400'}`}>
              Confirmação
            </span>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center">
            <img 
              ...
              className="h-14 sm:h-16 md:h-20 w-auto cursor-pointer hover:opacity-90 transition"
              onClick={() => setEtapa(1)}
            />
          </div>
          ...
          <button
            ...
            className="relative flex items-center gap-3 bg-white text-[#2d3e50] px-5 py-2.5 rounded-full shadow-md hover:shadow-lg transition-all hover:scale-105"
          >
            <svg 
            ...
            <span className="font-bold text-base hidden sm:inline">Carrinho</span>
            
            {carrinho.length > 0 && (
              <span className="absolute -top-2 -right-2 bg-orange-500 text-white text-sm font-bold px-2.5 py-1 rounded-full min-w-[28px] text-center shadow-md">
                {carrinho.length}
              </span>
            )}
          </button>
      ...
      {etapa === 1 && (
        <main className="bg-gray-100 flex-1">
          <div className="max-w-7xl mx-auto px-4 pb-8">
            {/* Categorias */}
            <div className="flex gap-3 overflow-x-auto whitespace-nowrap no-scrollbar py-3">
              {categorias.map(cat => (
                <button
                  key={cat}
                  onClick={() => setCategoriaFiltro(cat)}
                  className={`px-4 py-1.5 rounded-full font-semibold text-sm shrink-0 capitalize ${
                    categoriaFiltro === cat
                      ? 'bg-orange-500 text-white shadow'
                      : 'bg-gray-200 text-gray-700 hover:bg-[#0b3f65]'
                  }`}
                >
                  {cat}
                </button>
            ...
            {carregandoProdutos ? (
              ...
              <div className="text-center py-20">
                <p className="text-xl text-gray-600">Nenhum produto disponível.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-4">
                {produtosFiltrados.map(produto => (
                  <div key={produto.id} className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-all flex flex-col">
              ...
              >
                ...
                />
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      {/* BADGE CATEGORIA */}
                      <div className="mb-2">
                        <span className="inline-block bg-[#0d4a76] text-white px-3 py-1 rounded-full text-xs font-bold uppercase">
                          {produto.categoria}
                        </span>
                      </div>
                      ...
                      <div className="mt-auto flex items-end justify-between gap-3">
                        ...
                        <div className="flex-shrink-0">
                          ...
                          {produto.unidade && <span className="text-xs text-gray-500 mt-1">por {produto.unidade}</span>}
                        </div>
                        
                        {/* BOTÃO ADICIONAR */}
                        <button
                          onClick={() => adicionarAoCarrinho(produto)}
                          className={`flex-shrink-0 px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-1000 flex items-center gap-2 ${
      ...
      {/* ETAPA 2: CARRINHO */}
      {etapa === 2 && (
        <main className="bg-gray-100 py-6 flex-1">
          <div className="max-w-2xl mx-auto px-4">
            <div className="bg-[#0d4a76] text-white rounded-2xl p-4 md:p-6 flex justify-center items-center gap-3 mb-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
              {carrinho.length === 0 ? (
                ...
                <div className="text-center py-16">
                  <svg className="w-24 h-24 mx-auto mb-6 text-gray-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <circle cx="8" cy="21" r="1"/>
                    <circle cx="19" cy="21" r="1"/>
                    <path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12"/>
                  </svg>
                ...
                <>
                  {carrinho.map((item) => (
                    <div key={item.id} className="pb-4 mb-4 border-b">
                      ...
                      <p className="text-gray-600 text-sm mb-2">{item.unidade}</p>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <button 
                            ...
                            className="w-8 h-8 bg-gray-200 rounded-full font-bold text-lg flex items-center justify-center hover:bg-gray-300"
                          >
                            −
                          </button>
                          <span className="w-8 text-center font-bold">{item.qtd}</span>
                          <button 
                            onClick={() => atualizarQtd(item.id, item.qtd + 1)} 
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            ...
                            <p className="font-bold text-lg">R$ {(item.qtd * item.preco).toFixed(2)}</p>
                          </div>
                          <button 
                  ...
                  <div className="pt-4">
                    ...
                    <button 
                      onClick={() => setEtapa(1)} 
                      className="w-full mb-3 bg-gray-200 text-gray-700 py-3 rounded-lg font-semibold hover:bg-gray-300"
                    >
                      ← Continuar Comprando
                    </button>
                    ...
                    <button 
                      onClick={() => setEtapa(3)} 
                      className="w-full bg-[#0d4a76] text-white py-3 rounded-lg font-bold hover:bg-[#0b3f65] transition-colors duration-200"
        ...
        <main className="bg-gray-100 py-6 flex-1">
          <div className="max-w-2xl mx-auto px-4">
            <div className="bg-[#0d4a76] text-white rounded-2xl p-4 md:p-6 flex justify-center items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-bold">Dados para Entrega</h2>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 focus:outline-none"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 focus:outline-none resize-none"
                    rows={3}
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button 
                  ...
                  className="w-full bg-[#0d4a76] text-white py-3 rounded-lg font-bold hover:bg-[#0b3f65]"
                >
                  Continuar →
                </button>
              </div>
      ...
      <main className="bg-gray-100 py-6 flex-1">
        <div className="max-w-2xl mx-auto px-4">
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800 flex items-center justify-center mx-auto">Confirme seu Pedido</h2>
            ...
            <div className="mb-6">
              ...
              {/* SUBTOTAL */}
              <div className="border-t pt-2 mt-2 flex justify-between items-center text-sm text-gray-700">
                <span>Subtotal:</span>
            ...
            <div className="flex gap-3">
              ...
              <button 
                onClick={gerarPix}
                className="w-full bg-[#0d4a76] text-white py-3 rounded-lg font-bold hover:bg-[#0b3f65] flex items-center justify-center gap-2 transition-colors"
              >
      ...
      <main className="bg-gray-100 py-6 flex-1">
        <div className="max-w-3xl mx-auto px-4">
          <div className="bg-white rounded-2xl shadow-md overflow-hidden">
            ...
            {/* HEADER */}
            <div className="bg-[#0d4a76] text-white py-5 text-center">
            ...
            <div className="p-6 space-y-6">
              ...
              {carregandoPix ? (
                <div className="bg-green-50 rounded-lg p-8 text-center">
                  ...
                  <p className="font-semibold text-gray-700">Gerando pagamento PIX...</p>
                </div>
              ) : pixGerado && (
                <div className="bg-green-50 rounded-2xl p-6 space-y-6">
                  <h3 className="font-bold text-green-700 text-lg">Pagamento via PIX</h3>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-xl p-4 text-center">
                      ...
                      <p className="text-sm text-gray-600 mt-3">Escaneie com o app do seu banco</p>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <button
                        ...
                        className="w-full bg-green-600 hover:bg-green-700 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors"
                      >
                        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {/* AGUARDANDO */}
                  <div className="bg-yellow-100 border-l-4 border-yellow-500 rounded-lg p-4 font-semibold text-yellow-800 flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="w-full bg-gray-200 hover:bg-gray-300 py-3 rounded-xl font-semibold transition-colors"
                  >
                    Cancelar
                  </button>
    ...
    {mostrarToast && (
      <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-slide-down">
        <div className="bg-[#0d4a76] text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 border-2 border-white/20 max-w-md">
          <span className="font-semibold text-sm">{mensagemToast}</span>
        </div>
      </div>
    )}
